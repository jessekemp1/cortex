...
try:
    ...
    orch_spec = importlib.util.spec_from_file_location(
        "lo_orchestrator", Path(LOCAL_ORCH_PATH) / "orchestrator.py"
    )
...
try:
    ...
    cortex_orch_spec = importlib.util.spec_from_file_location(
        "cortex_orchestrator", Path(__file__).parent.parent / "orchestrator.py"
    )
...
class RecommendationToAgentAdapter:
    ...
    def to_agent(self, recommendation: Recommendation) -> BaseAgent:
        ...
        if not LOCAL_ORCHESTRATOR_AVAILABLE or ScheduledTaskAgent is None:
            raise RuntimeError(
                "local-orchestrator not available. Install dependencies: pip install structlog apscheduler fastapi uvicorn"
            )
        ...
        action_title = getattr(
            recommendation,
            "title",
            getattr(recommendation, "action_title", "Unknown Action"),
        )
    ...
    def to_schedule(
        self, recommendation: Recommendation, default_schedule: str = "0 8 * * *"
    ) -> str:
        """
        ...
        priority = getattr(recommendation, "priority", "medium")
        title = getattr(
            recommendation, "title", getattr(recommendation, "action_title", "")
        ).lower()
    ...
    def register_recommendation(
        self, recommendation: Recommendation, schedule: Optional[str] = None
    ) -> bool:
        """
...
class CortexLocalOrchestratorIntegration:
    ...
    def _initialize(self):
        ...
        try:
            self.adapter = RecommendationToAgentAdapter(
                None
            )  # No orchestrator needed for conversion
        except Exception as e:
    ...
    def execute_recommendation(self, recommendation: Recommendation) -> Dict[str, Any]:
        ...
        try:
            ...
            if getattr(recommendation, "type", "") == "cursorrules_improvement":
                ...
                project_name = (
                    getattr(recommendation, "related_projects", [])[0]
                    if getattr(recommendation, "related_projects", [])
                    else None
                )
    ...
    def list_scheduled_actions(self) -> list:
        ...
        try:
            ...
            cortex_agents = [
                a for a in agents if a.get("agent_id", "").startswith("cortex_")
            ]
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        """
        Get statistics for the Symbiosis Dashboard.
        Returns queue stats and recent activity.
        """
        if not self.is_available() or not self.orchestrator:
            return {"error": "Local Orchestrator not available"}

        stats = {"queue": {"pending": 0, "active_batches": 0}, "recent_activity": []}

        try:
            # 1. Batch Queue Stats
            if hasattr(self.orchestrator, "batch_manager"):
                manager = self.orchestrator.batch_manager
                pending = manager.get_pending_items(limit=1)  # Just check count?
                # get_pending_items returns list.
                # Ideally we want a count method, but len(get_pending(limit=1000)) is fine for now
                pending_all = manager.get_pending_items(limit=1000)
                active_batches = manager.get_active_batches()

                stats["queue"]["pending"] = len(pending_all)
                stats["queue"]["active_batches"] = len(active_batches)
                stats["queue"]["batches"] = active_batches  # Include details

            # 2. Recent Activity
            history = self.orchestrator.history.get_recent_executions(limit=10)
            stats["recent_activity"] = history
