"""Briefing generator — produces the BriefingData payload from cross-project data.

This module hosts the BriefingGenerator class (~1217 LOC) and its public
convenience wrapper `generate_daily_briefing`. It was extracted from
briefing/__init__.py as the final step of the Phase 3b split (see
`docs/AUDIT_FINDINGS.md`). After this move, briefing/__init__.py becomes
a thin shim: heavy try/except dependency-import block, style helpers,
BriefingData dataclass, and re-exports.

The generator depends on a chain of optional helpers (ProjectScanner,
GoalParser, RecommendationEngine, GitTracker, ProcessMonitor, …) imported
inside briefing/__init__.py with try/except, so any can be None when its
module isn't available. All accesses are routed through the briefing
package namespace via `_briefing.X` so existing tests that patch
`briefing.GitTracker` / `briefing.ProcessMonitor` continue to intercept
the dynamic lookups at call time.
"""
...
if TYPE_CHECKING:
    # Type-only imports for editor/mypy. With `from __future__ import annotations`
    # active, runtime annotation evaluation is deferred to string-resolution time,
    # so these don't need to be importable at module load.
    from . import BriefingData, Goal, ProjectActivity, Recommendation

# Package-namespace alias for runtime dispatch through patchable attributes
# (GitTracker, ProcessMonitor, etc. — see `tests/test_briefing.py` which
# applies `patch("briefing.GitTracker", None)` before invoking the generator).
...
class BriefingGenerator:
    ...
    def __init__(self, root_dir: Optional[Path] = None):
        ...
        self.project_scanner = _briefing.ProjectScanner(str(root_dir)) if _briefing.ProjectScanner else None
        self.goal_parser = _briefing.GoalParser() if _briefing.GoalParser else None
        self.recommendation_engine = _briefing.RecommendationEngine() if _briefing.RecommendationEngine else None

        # Initialize enhanced intelligence tools
        self.learning_system = _briefing.LearningSystem() if _briefing.LearningSystem else None
        self.portfolio_memory = _briefing.PortfolioMemory() if _briefing.PortfolioMemory else None
        self.session_manager = _briefing.SessionManager(root_dir) if _briefing.SessionManager else None

        # Initialize resource & orchestration tools
        self.usage_optimizer = _briefing.UsageOptimizer() if _briefing.UsageOptimizer else None
        self.metrics_tracker = _briefing.MetricsTracker() if _briefing.MetricsTracker else None
        self.contract_metrics_store = _briefing.ContractMetricsStore() if _briefing.ContractMetricsStore else None
    ...
    def generate_daily_briefing(self) -> BriefingData:
        ...
        git_status = None
        if _briefing.GitTracker:
            try:
                tracker = _briefing.GitTracker(str(self.root_dir))
                git_status = {
        ...
        if _briefing.ProcessMonitor:
            try:
                monitor = _briefing.ProcessMonitor()
        ...
        briefing = _briefing.BriefingData(
    ...
    def _get_velocity_metrics(self) -> Optional[Dict[str, Any]]:
        """Get velocity and ROI metrics from _briefing.MetricsTracker."""
    ...
    def _get_queue_slo_metrics(self) -> Optional[Dict[str, Any]]:
        ...
        if not _briefing.check_queue_slo:
            return None
        try:
            return _briefing.check_queue_slo()
        except Exception as e:
