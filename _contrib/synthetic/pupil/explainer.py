...
_FACTOR_SPECS: Dict[str, Dict[str, dict]] = {
    ActionType.CHURN.value: {
        "satisfaction": {
            ...
            "weight": 0.30,
            "label_fn": lambda v: (
                f"satisfaction at {v:.2f}" + (" (below risk threshold 0.50)" if v < 0.5 else "")
            ),
        },
        "monthly_churn_prob": {
        ...
        "tenure_years": {
            ...
            "weight": 0.15,
            "label_fn": lambda v: (
                f"tenure {v:.1f} years" + (" (short tenure increases risk)" if v < 3.0 else "")
            ),
        },
        "num_products": {
            ...
            "weight": 0.10,
            "label_fn": lambda v: (
                f"{int(v)} products held" + (" (single product = less sticky)" if v < 2 else "")
            ),
        },
        "tenure_modifier": {
            ...
            "weight": 0.10,
            "label_fn": lambda v: (
                f"tenure modifier {v:.2f}" + (" (loyalty reducing churn)" if v < 0.7 else "")
            ),
        },
        "product_modifier": {
            "direction_fn": lambda v: "protective" if v < 0.7 else "risk",
    ...
    ActionType.MISS_PAYMENT.value: {
        "credit_score": {
            ...
            "weight": 0.50,
            "label_fn": lambda v: (
                f"credit score {int(v)}" + (" (below prime threshold)" if v < 650 else "")
            ),
        },
        "miss_threshold": {
