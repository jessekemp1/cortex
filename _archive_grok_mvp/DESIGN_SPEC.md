...
|---------|-------------------|------------------------|--------------|
| **Know what to do next** | `converx next` command | Returns prioritized recommendation with rationale | User executes action |
...
| **Verify system is working** | System health check | Shows which integrations are active | `converx health` command |
| **Close the verification loop** | Feedback logger | Captures user feedback on recommendations | `converx feedback` command + stats |
| **Focus on specific project** | Project filtering | `converx next PROJECT` filters recommendations | User gets project-specific actions |
...
| **Understand current state** | Status command | `converx status` shows projects, goals, blockers | User sees system state |

**Outcome Statement**: "When I run `converx next`, I receive a clear, prioritized next action with transparent rationale, system health status, and the ability to provide feedback for continuous improvement."
...
converx next --scenarios           # Include scenario bands with recommendation
