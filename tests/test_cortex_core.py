...
[ERROR] cli.py:100 - ValueError: invalid option index
    ...
    def test_build_context(self, context_builder):
        ...
        with patch.object(context_builder, "get_git_status") as mock_git, patch.object(
            context_builder, "get_recent_errors"
        ) as mock_errors, patch.object(
            context_builder, "get_test_results"
        ) as mock_tests:
...
class TestDecisionLogger:
    ...
    def test_get_decision(self, decision_logger, sample_context, sample_options):
        ...
        decision_id = decision_logger.log_decision(
            context=sample_context, options=sample_options, selected_option=1
        )
    ...
    def test_get_recent_decisions(
        self, decision_logger, sample_context, sample_options
    ):
        """Test retrieving recent decisions."""
        # Log multiple decisions
        for i in range(5):
            decision_logger.log_decision(
                context=sample_context, options=sample_options, selected_option=i % 3
            )
...
class TestOutcomeTracker:
    ...
    def test_track_outcome_success(
        self, outcome_tracker, decision_logger, sample_context, sample_options
    ):
        """Test tracking successful outcome."""
        decision_id = decision_logger.log_decision(
            context=sample_context, options=sample_options, selected_option=0
        )
    ...
    def test_track_outcome_failure(
        self, outcome_tracker, decision_logger, sample_context, sample_options
    ):
    ...
    def test_get_decision_outcomes(
        self, outcome_tracker, decision_logger, sample_context, sample_options
    ):
...
class TestStrategicOptions:
    ...
    def test_generate_options_with_errors(self, strategic_options):
        ...
        context = {
            ...
            "recent_errors": [{"file": "main.py", "line": 10, "error": "SyntaxError"}],
        ...
        assert any(
            "error" in opt["title"].lower() or "error" in opt["description"].lower()
    ...
    def test_generate_options_with_failing_tests(self, strategic_options):
        ...
        assert any(
            "test" in opt["title"].lower() or "test" in opt["description"].lower()
            for opt in options
        )
    ...
    def test_select_option(self, strategic_options, sample_context, sample_options):
        ...
        with patch.object(
            strategic_options, "generate_options", return_value=sample_options
        ):
            decision_id = strategic_options.select_option(
                context=sample_context, option_index=0, reasoning="Most critical issue"
            )

            assert decision_id > 0

    def test_select_invalid_option(
        self, strategic_options, sample_context, sample_options
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(IndexError):
                strategic_options.select_option(context=sample_context, option_index=99)

    def test_learning_improves_recommendations(
        self, strategic_options, decision_logger, outcome_tracker
    ):
        ...
        for _ in range(5):
            ...
            outcome_tracker.track_outcome(
                decision_id, "success", {"time_taken": "20 min"}
            )
