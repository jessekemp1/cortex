...
class TestMcpBridgeTimeouts:
    """Any tool that still fronts the bridge over HTTP must use a timeout that
    exceeds the measured latency of the endpoint it fronts (e.g. /graph/query).

    As of release/1.2.0 the core memory-loop tools (cortex_intelligence,
    cortex_recommendations) run IN-PROCESS via the CortexBridge singleton and no
    longer make an HTTP round-trip — so a short-proxy-timeout regression is
    architecturally impossible for them. The in-process contract is asserted
    directly below; the raised-timeout invariant is retained for the tools that
    remain bridge-proxied (cortex_graph_query)."""
    ...
    def test_intelligence_runs_in_process_not_over_http(self, mcp_module, monkeypatch):
        # cortex_intelligence must NOT make an HTTP round-trip to the bridge —
        # it runs in-process via the CortexBridge singleton. If it ever calls
        # _bridge_post/_bridge_get again, the crash-proof "works with the bridge
        # down" guarantee regresses.
        def _boom_post(*a, **k):
            raise AssertionError("cortex_intelligence must not call _bridge_post")

        def _boom_get(*a, **k):
            raise AssertionError("cortex_intelligence must not call _bridge_get")
        ...
        class _FakeBridge:
            ...
            def query_intelligence(self, request, project=None, query_type="research"):
                called["request"] = request
                return {"ok": True}

        monkeypatch.setattr(mcp_module, "_bridge_post", _boom_post)
        monkeypatch.setattr(mcp_module, "_bridge_get", _boom_get)
        monkeypatch.setattr(mcp_module, "_get_bridge", lambda: _FakeBridge())
        fn = self._resolve_tool(mcp_module, "cortex_intelligence")
        out = fn(query="q")
        assert called["request"] == "q"
        assert "ok" in out

    def test_recommendations_runs_in_process_not_over_http(self, mcp_module, monkeypatch):
        # cortex_recommendations is also in-process (bridge.get_recommendations +
        # mcp_handlers.normalize_recommendations), no HTTP proxy.
        def _boom_get(*a, **k):
            raise AssertionError("cortex_recommendations must not call _bridge_get")
        ...
        monkeypatch.setattr(mcp_module, "_bridge_get", _boom_get)
        monkeypatch.setattr(mcp_module, "_get_bridge", lambda: _FakeBridge())
        out = self._resolve_tool(mcp_module, "cortex_recommendations")()
        assert isinstance(out, str)

    def test_graph_uses_raised_timeout(self, mcp_module, monkeypatch):
        # cortex_graph_query remains bridge-proxied — its timeout must exceed the
        # measured endpoint latency so a slow graph scan doesn't fail by default.
        captured = {}
