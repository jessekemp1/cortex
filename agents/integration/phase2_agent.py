...
class Phase2Agent(BaseCoordinationAgent):
    ...
    def __init__(self, root_dir: Path = None):
        super().__init__(
            phase_name="Add Integration Layer",
            phase_number=2,
            root_dir=root_dir
        )
...
"""
    ...
    def _add_schedule_command(self):
        ...
        if "cmd_schedule" in content or "schedule" in content and "subparsers.add_parser('schedule')" in content:
            return  # Already added
        ...
        if "from integration.local_orchestrator import" not in content:
            ...
            for i, line in enumerate(lines):
                if line.startswith("from feedback import"):
                    lines.insert(i + 1, "from integration.local_orchestrator import CortexLocalOrchestratorIntegration")
                    break
            content = '\n'.join(lines)
