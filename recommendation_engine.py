...
class Recommendation:
    ...
    id: str
    title: str
    type: str  # "blocker_resolution", "goal_progress", "project_health", "momentum", etc.
...
class RecommendationEngine:
    ...
    def generate_recommendations(
        ...
        if project_activity:
            recommendations.extend(self._generate_health_recommendations(project_activity))
    ...
    def _apply_learning_adjustments(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        """
    ...
    def _generate_goal_recommendations(
        ...
        for goal in goals:
            ...
            rec_priority = {'A': 'high', 'B': 'medium', 'C': 'low'}.get(priority, 'medium')
...
• Decide: continue, pause, or archive"""
    ...
    def _generate_momentum_recommendations(
        ...
        active_projects = [p for p in project_activity if getattr(p, 'status', '') == 'active']
        ...
        for project in active_projects[:3]:  # Limit to top 3 active
            ...
            if last_msg:
                rationale += f" Last: '{last_msg[:50]}...'" if len(last_msg) > 50 else f" Last: '{last_msg}'"
...
• Continue from last commit"""
    ...
    def _is_important_project(self, project_name: str) -> bool:
        ...
        important = {
            'vortexv2', 'alpha_arena', 'personal-ai-dataset', 'cortex',
            'keto-tracker', 'windfield', 'vortex', 'local-orchestrator'
        }
...
def main():
    ...
    if args.json:
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            print(f"   Type: {rec.type} | Priority: {rec.priority} | Confidence: {rec.confidence:.0%}")
