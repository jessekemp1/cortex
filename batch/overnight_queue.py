...
def test_validation_jobs() -> List[LocalJob]:
    ...
    suites = [
        ("Vortex Backend", "Vortex/backend/tests", "pytest Vortex/backend/tests/ -x -q"),
        ("Winfield", "Vortex/Winfield/tests", "pytest Vortex/Winfield/tests/ -x -q"),
        ("Alpha Arena", "alpha_arena/tests", "pytest alpha_arena/tests/ -x -q"),
        ("Cortex", "cortex/tests", "pytest cortex/tests/ -x -q"),
        ("Pupil", "pupil/tests", "pytest pupil/tests/ -x -q"),
    ]
    jobs = []
    for name, test_dir, command in suites:
...
def emos_monitoring_job() -> LocalJob:
    """Check EMOS pair counts — tracks readiness for calibration."""
    return LocalJob(
        description="EMOS pair count check (HRRR/GFS/ECMWF)",
        command=(
            'python3 -c "'
            "from app.core.db_connection import get_session;"
            "from app.core.models import ValidationPair;"
            "from sqlalchemy import func;"
            "from datetime import datetime, timedelta;"
            "s = next(get_session());"
            "cutoff = datetime.utcnow() - timedelta(days=30);"
            "counts = s.query(ValidationPair.model_name, func.count())"
            ".filter(ValidationPair.created_at >= cutoff)"
            ".group_by(ValidationPair.model_name).all();"
            'print(dict(counts))"'
        ),
        task_type="monitoring",
        working_dir=WORKSPACE / "Vortex" / "backend",
        priority="normal",
        timeout_seconds=30,
