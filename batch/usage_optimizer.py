...
class UsageOptimizer:
    ...
    def get_compliance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._get_recommendations(
                daily_real_time_hours,
                summary['totals']['batch_percentage']
            )
        }
    ...
    def _get_recommendations(
        ...
        if daily_hours > self.targets.daily_sustainable_hours:
            ...
            recs.append(
                f"⚠️  Reduce real-time usage by {excess:.1f} hours/day to meet target"
            )

        if batch_pct < self.targets.target_batch_percentage:
            gap = self.targets.target_batch_percentage - batch_pct
            recs.append(
                f"📊 Increase batch usage by {gap:.1f}% (currently {batch_pct:.1f}%)"
            )

        if batch_pct >= self.targets.target_batch_percentage and daily_hours <= self.targets.daily_sustainable_hours:
...
def generate_optimization_report():
    ...
    print(f"Status:                 {'✅ WITHIN TARGET' if compliance['current']['within_target'] else '⚠️  OVER TARGET'}")
    print()
    ...
    print(f"Status:                 {'✅ ON TRACK' if compliance['batch_usage']['on_track'] else '📈 NEEDS IMPROVEMENT'}")
    print()
    ...
    if summary['optimization']['missed_opportunities'] > 0:
        ...
        print("-" * 70)
        print(f"Tasks that could have been batch: {summary['optimization']['missed_opportunities']}")
        print(f"Potential additional savings:     ${summary['optimization']['potential_additional_savings']:.2f}")
        print()
