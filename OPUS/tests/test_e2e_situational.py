...
def run_cli_command(cmd_args: list, timeout: int = 10) -> Tuple[str, str, int, float]:
    ...
    cmd = [sys.executable, "-m", "converx.cli", "--root", str(root_dir)] + cmd_args
...
def test_json_output_format():
    ...
    assert isinstance(data["alternative_actions"], list), \
        "alternative_actions should be a list"
    
    # Validate context_predictions
    assert isinstance(data["context_predictions"], list), \
...
def test_status_command():
    ...
    assert "Total:" in stdout or "Active" in stdout, \
        "Output should show project counts"
    
    # Should show goal progress
    assert "Priority" in stdout or "in progress" in stdout or "pending" in stdout, \
