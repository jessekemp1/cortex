...
if TYPE_CHECKING:
    from .metric_tracker import MetricRecord, MetricTracker
...
class TrendAnalyzer:
    ...
    def _get_metrics_for_period(
        self, project: str, metric_type: str, days: int
    ) -> List["MetricRecord"]:
        """
        ...
        return self.metric_tracker.get_metrics(project=project, metric_type=metric_type, days=days)
    ...
    def _calculate_confidence(self, data_points: int, r_squared: float, days: int) -> float:
        """
    ...
    def detect_anomalies(self, project: str, metric_type: str, days: int = 7) -> List[Anomaly]:
        """
        ...
        anomalies.sort(key=lambda a: (0 if a.severity == AlertLevel.CRITICAL else 1, -a.deviation))
    ...
    def get_project_health_summary(self, project: str, days: int = 7) -> dict:
        ...
        critical_count = sum(1 for t in trends.values() if t.alert_level == AlertLevel.CRITICAL)
        warning_count = sum(1 for t in trends.values() if t.alert_level == AlertLevel.WARNING)
        critical_anomalies = sum(1 for a in all_anomalies if a.severity == AlertLevel.CRITICAL)
