...
class ResourceOptimizer:
    ...
    def detect_waste(self) -> List[WasteItem]:
        ...
        high_mem_idle = [
            ...
            if p.memory_mb > 1000.0 and p.cpu_percent < 1.0 and p.category not in (ProcessCategory.SYSTEM, ProcessCategory.OTHER)
        ]
        for proc in high_mem_idle:
            waste_items.append(WasteItem(
                ...
                auto_actionable=False,
                metadata={"memory_mb": proc.memory_mb, "category": proc.category.value},
            ))
        ...
        for name, count in process_counts.items():
            if count > 1:
                ...
                waste_items.append(WasteItem(
                    ...
                    auto_actionable=False,
                    metadata={"instance_count": count, "total_memory_mb": total_memory},
                ))
    ...
    def suggest_optimizations(self) -> List[Optimization]:
        ...
        if insights.peak_hours and insights.idle_hours:
            peak_avg = sum(insights.avg_cpu_by_hour.get(h, 0) for h in insights.peak_hours) / len(insights.peak_hours)
            idle_avg = sum(insights.avg_cpu_by_hour.get(h, 0) for h in insights.idle_hours) / len(insights.idle_hours)
        ...
        for ai_tool in ai_insights:
            if ai_tool.idle_hours > 2.0 and ai_tool.avg_memory > 500:
                optimizations.append(Optimization(
                    ...
                    metadata={"tool": ai_tool.tool_name, "idle_hours": ai_tool.idle_hours},
                ))
        ...
        if current.total_cpu_percent > 70:
            ...
            if high_cpu_procs:
                ...
                optimizations.append(Optimization(
                    ...
                    metadata={"top_process": top_proc.name, "cpu_percent": top_proc.cpu_percent},
                ))

        return sorted(optimizations, key=lambda o: {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}.get(o.priority, 4))
