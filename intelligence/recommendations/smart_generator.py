...
class SmartRecommendationGenerator:
    ...
    def generate_steps(
        ...
        steps.extend(self._generate_context_specific_steps(
            recommendation_type, context, len(steps)
        ))
    ...
    def enrich_with_intelligence(self, recommendation: Recommendation) -> Recommendation:
        ...
        if not recommendation.similar_work:
            recommendation.similar_work = self._find_similar_patterns(
                ...
                recommendation.intelligence_context
            )
    ...
    def generate_alert_recommendations(
        ...
        for alert_type, type_alerts in alerts_by_type.items():
            ...
            file_guidance = self._generate_file_guidance(
                alert_type, context, intelligence
            )
    ...
    def _estimate_alert_impact(self, alerts: List[Alert]) -> str:
        ...
        total_score = sum(
            severity_scores.get(a.severity, 1) for a in alerts
        )
    ...
    def generate_goal_recommendations(
        ...
        for goal in goals:
            ...
            # Generate file guidance
            file_guidance = self._generate_goal_file_guidance(
                goal, project_activity, intelligence
            )
    ...
    def _check_dependency_health(
        ...
        if outdated > 5:
            return Recommendation(
                ...
                confidence=Confidence.MEDIUM,
                steps=[
                    Step(1, "Review outdated dependencies", "pip list --outdated", "List generated"),
                    Step(2, "Update dependencies incrementally", "pip install --upgrade <package>", "Package updated"),
                    Step(3, "Run tests after each update", "pytest", "All tests pass")
                ],
                intelligence_context={"project_id": activity.project_id, **intelligence},
    ...
    def _check_code_quality_health(
        ...
        if not quality_tools:
            return Recommendation(
                ...
                confidence=Confidence.MEDIUM,
                steps=[
                    Step(1, "Install ruff for linting", "pip install ruff", "Ruff installed"),
                    Step(2, "Install mypy for type checking", "pip install mypy", "Mypy installed"),
                    Step(3, "Create configuration", "touch pyproject.toml", "Config file created"),
                    Step(4, "Run initial quality check", "ruff check . && mypy .", "No critical issues")
                ],
                file_guidance=[
                    FileGuidance(
    ...
    def _check_hotspot_health(
        ...
        if len(hotspots) > 3:
            return Recommendation(
                ...
                steps=[
                    Step(1, "Analyze change frequency", "git log --format='%H' --follow -- <file> | wc -l", "Change count obtained"),
                    Step(2, "Review hotspot complexity", None, "Complexity issues identified"),
                    Step(3, "Plan refactoring if needed", None, "Refactoring plan created")
                ],
                file_guidance=[
                    FileGuidance(
                        file_path=hotspot,
                ...
                intelligence_context={"project_id": activity.project_id, "hotspots": hotspots, **intelligence},
                estimated_impact="medium",
    ...
    def generate_all_recommendations(
        ...
        if alerts:
            all_recommendations.extend(
                self.generate_alert_recommendations(alerts)
            )
        
        # Generate goal recommendations
        if goals:
            all_recommendations.extend(
                self.generate_goal_recommendations(goals, activity)
            )
        
        # Generate health recommendations
        all_recommendations.extend(
            self.generate_health_recommendations(activity)
        )
