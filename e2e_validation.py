...
class E2EValidator:
    ...
    def test_performance(self) -> str:
        ...
        assert benchmarks['portfolio_stats_ms'] < 100, f"Portfolio stats too slow: {benchmarks['portfolio_stats_ms']:.1f}ms"
        ...
        assert benchmarks['session_context_ms'] < 500, f"Session context too slow: {benchmarks['session_context_ms']:.1f}ms"

        # Benchmark: Spec search
        start = time.time()
        results = self.specs.search("test query", limit=5)
        benchmarks['spec_search_ms'] = (time.time() - start) * 1000
        assert benchmarks['spec_search_ms'] < 1000, f"Spec search too slow: {benchmarks['spec_search_ms']:.1f}ms"

        # Benchmark: Metrics dashboard
        start = time.time()
        dashboard = self.metrics.get_dashboard()
        benchmarks['metrics_dashboard_ms'] = (time.time() - start) * 1000
        assert benchmarks['metrics_dashboard_ms'] < 100, f"Metrics dashboard too slow: {benchmarks['metrics_dashboard_ms']:.1f}ms"
