...
def parse_project_status(root_dir: Path) -> Dict[str, Any]:
    ...
    total_projects = (
        ...
        if total_match
        else (active_projects + backlog_projects + inactive_projects)
    )

    # Extract active project names
    # Look for numbered list under "## 🟢 ACTIVE"
    active_section_match = re.search(r"## 🟢 ACTIVE.*?\n(.*?)(?=##|\Z)", content, re.DOTALL)
...
def get_strategic_context(root_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    return {**project_metrics, **goal_metrics, "root_dir": str(root_dir)}
