...
**Cortex** is a strategic orchestrator that answers: "What should I do next?"
...
**Key Insight**: 80% of functionality already exists in the codebase. Cortex is a thin orchestration layer that unifies:
...
**Golden Spec Alignment**: This design follows the Golden Spec Method framework, ensuring traceability from outcomes to solutions and verification loops for continuous improvement.
...
**What's Included**:
- ✅ CLI interface (`cortex next [PROJECT]`)
...
1. **User runs**: `cortex next`
2. **CLI calls**: `CortexOrchestrator.get_next_action()`
3. **Orchestrator**:
...
1. Single command (`cortex next`) returns actionable next step
2. Output is more useful than running `recommendation_engine.py` directly
3. Can focus on specific project (`cortex next PROJECT`)
4. Takes <5 seconds to run
5. No new dependencies required
6. Integrates cleanly with existing tools

### Traceability Matrix (Golden Spec: Phase 5 Alignment)

**Purpose**: Map every solution component to the outcome it achieves, ensuring no gaps or over-engineering.

| Outcome | Solution Component | How It Achieves Outcome | Verification |
|---------|-------------------|------------------------|--------------|
| **Know what to do next** | `cortex next` command | Returns prioritized recommendation with rationale | User executes action |
| **Understand why this action** | Enhanced rationale with decision factors | Shows priority, impact, confidence weights | User sees traceability in output |
| **Verify system is working** | System health check | Shows which integrations are active | `cortex health` command |
| **Close the verification loop** | Feedback logger | Captures user feedback on recommendations | `cortex feedback` command + stats |
| **Focus on specific project** | Project filtering | `cortex next PROJECT` filters recommendations | User gets project-specific actions |
| **Get context when needed** | Context integration | `--with-context` flag includes context predictions | User sees relevant context |
| **Programmatic access** | JSON output | `--json` flag for automation | Other tools can consume output |
| **Understand current state** | Status command | `cortex status` shows projects, goals, blockers | User sees system state |

**Outcome Statement**: "When I run `cortex next`, I receive a clear, prioritized next action with transparent rationale, system health status, and the ability to provide feedback for continuous improvement."

**Success Metrics**:
- Recommendations actionable 70%+ of time (measured via feedback logger)
- System health visible in every output
- Feedback loop closed (user can log feedback, view stats)
- Traceability visible (decision factors shown in output)
...
**Cortex becomes your "Strategic Co-Pilot" - a system that understands your work context, predicts what you need, and guides you toward optimal decisions across all life domains.**
...
**Example**:
```bash
cortex simulate "Complete VortexV2 Block 1.2"
...
**Example**:
```bash
cortex forecast "VortexV2 MVP Completion"
# Output:
...
#### 3. Multi-Domain Life Weather Map
...
**Example**:
```bash
cortex route "VortexV2 MVP Completion"
# Output:
# Route: 5 waypoints
...
- **Config File**: Optional `~/.cortex/config.json` for preferences
...
- `GET /api/v1/weather` - Life weather map

#### 3. Web Interface

**Future**: Streamlit dashboard for:
- Visual Life Weather Map
- Route visualization
- Scenario comparison
- Historical patterns
...
**Given**: User runs `cortex next vortexv2`  
...
**Given**: User runs `cortex next --json`  
...
**Given**: User runs `cortex next --with-context`  
...
**Given**: User runs `cortex simulate "Complete VortexV2 Block 1.2"`  
...
**Given**: User runs `cortex forecast "VortexV2 MVP Completion"`  
...
#### Test Case 11: Life Weather Map

**Given**: User runs `cortex weather`  
...
**Given**: User runs `cortex route "VortexV2 MVP Completion"`  
...
**Given**: Cortex calls ProjectScanner  
**When**: Multiple projects exist  
**Then**:
...
**Given**: Cortex calls RecommendationEngine  
...
**Location**: `cortex/ARCHITECTURE.md` (optional for MVP, required for full product)
...
**Required Sections**:
- Getting Started
- Core Concepts (virtual twin, scenarios, weather map)
- Advanced Usage
- Best Practices
- FAQ

**Location**: `cortex/docs/USER_GUIDE.md`
...
**Location**: `cortex/docs/DEVELOPER_GUIDE.md`
...
- Life Weather Map Design
- Route System Design

**Location**: `cortex/docs/DESIGN/`
...
**What Was Built**:
- `cortex next` - Prioritized next action
- `cortex status` - Current state summary
- Project filtering, JSON output, context integration
- 11 tests passing

---

## Phase 1: Weather Map + Scenario Bands

### Phase 1 Design

**Goal**: Add strategic framing with weather metaphors and scenario bands
...
class ScenarioForecast:
    """Complete forecast with all scenario bands."""
    goal: str
...
```bash
cortex weather                    # Show current weather for Work/Code
cortex weather --all              # Show all domains
cortex next --scenarios           # Include scenario bands with recommendation
cortex complete WAYPOINT_ID       # Mark waypoint as complete
```
...
#### Test Case P1.3: Scenario Bands - All Three Present

**Given**: A goal with estimated effort of 2 weeks

**When**: `cortex next --scenarios` is run
...
**When**: `cortex complete WAYPOINT_3` is run
...
```bash
cortex route "Ship VortexV2 MVP"    # Create/view route
cortex route --list                  # List all active routes
cortex route ROUTE_ID --detail       # Detailed route view
cortex forecast ROUTE_ID             # Scenario forecast for route
cortex domains                       # Show all domain weather
cortex domains work_code             # Show specific domain
```
...
**When**: `cortex route "Ship VortexV2 MVP"` is run
...
**When**: `cortex route ROUTE_ID` is run
...
```bash
cortex connectors                    # List available connectors
cortex connectors --status           # Show connector health
cortex search "query"                # Search across all sources
cortex search "query" --domain work  # Search specific domain
cortex sync                          # Refresh all connector data
```
...
**When**: `cortex domains health` is run
...
**When**: `cortex search "VortexV2 forecasting"` is run
...
**When**: `cortex connectors --status` is run
...
```bash
cortex playbooks                     # List available playbooks
cortex run analyze_repo              # Run a playbook
cortex run analyze_repo --dry-run    # Show what would execute
cortex executions                    # List recent executions
cortex executions EXEC_ID            # Show execution details
cortex policies                      # Show current policies
cortex approve EXEC_ID               # Approve pending execution
```
...
**When**: `cortex run analyze_repo` is executed
...
**When**: `cortex run PLAYBOOK --dry-run` is executed
...
```bash
cortex simulate ROUTE_ID             # Simulate route outcomes
cortex simulate ROUTE_ID --monte-carlo  # Full probabilistic simulation
cortex twin                          # Show current twin state
cortex twin --history                # Show state over time
cortex reflect                       # Run reflection analysis
cortex reflect --predictions         # Show prediction accuracy
cortex calibrate                     # Recalibrate models from data
```
...
When evidence conflicts, Cortex shows you - no hiding from reality.
...
## The Core Question: How Does Cortex Ensure You Win?
...
Cortex shows you what you wouldn't see until it's too late.
```
...
```
$ cortex decide "Should I take the meeting with potential investor?"
...
You run `cortex next` and get a clear action with rationale. You didn't have to set up anything, define goals, or organize tasks. It pulled from your existing work (git repos, ACTION_PLAN.md) and synthesized.
...
Cortex shows you anyway - not judgmentally, just factually. And somehow, facing truth is easier when it comes from a system than from yourself.

**4. Reduces Cognitive Load, Doesn't Add It**

You're not managing Cortex. Cortex is managing the complexity. You show up, it tells you what matters. You execute, it tracks. You reflect, it learns.
...
Cortex is not a productivity tool. It's a **cognitive exoskeleton** for your strategic mind.
...
Use Cortex for one week. Just `cortex next` each morning and `cortex reflect` each evening.
...
**Key Principle**: Each phase delivers standalone value. Stop at any phase. Don't build Virtual Twin before validating Routes. Don't build Routes before validating Weather Map.
