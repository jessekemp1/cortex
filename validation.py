...
class InputValidator:
    ...
    def validate_project_name(self, name: str, allow_empty: bool = False) -> ValidationResult:
        """
        ...
        result = ValidationResult(
            is_valid=True, validation_type=ValidationType.PROJECT_NAME, original_value=name
        )
    ...
    def validate_file_path(
        ...
        """
        result = ValidationResult(
            is_valid=True, validation_type=ValidationType.FILE_PATH, original_value=path
        )
    ...
    def validate_directory_path(
        ...
        """
        result = ValidationResult(
            is_valid=True, validation_type=ValidationType.DIRECTORY_PATH, original_value=path
        )
        ...
        if len(path) > self.MAX_PATH_LENGTH:
            result.add_error(
                f"Directory path exceeds maximum length of " f"{self.MAX_PATH_LENGTH} characters"
            )
    ...
    def validate_identifier(self, identifier: str, allow_empty: bool = False) -> ValidationResult:
        """
        ...
        """
        result = ValidationResult(
            is_valid=True, validation_type=ValidationType.IDENTIFIER, original_value=identifier
        )
        ...
        if len(identifier) > self.MAX_IDENTIFIER_LENGTH:
            result.add_error(
                f"Identifier exceeds maximum length of " f"{self.MAX_IDENTIFIER_LENGTH} characters"
            )
    ...
    def _check_security_patterns(self, value: str, result: ValidationResult) -> None:
        """
    ...
    def _check_path_traversal(self, path: str, result: ValidationResult) -> None:
        """
        ...
        for pattern in self.PATH_TRAVERSAL_PATTERNS:
            if pattern.search(path):
                result.add_error(
                    f"Path contains potential path traversal pattern: " f"{pattern.pattern}"
                )

    def _check_command_injection(self, value: str, result: ValidationResult) -> None:
        """
        ...
        for pattern in self.COMMAND_INJECTION_PATTERNS:
            if pattern.search(value):
                result.add_error(
                    f"Input contains potential command injection pattern: " f"{pattern.pattern}"
                )

    def _check_sql_injection(self, value: str, result: ValidationResult) -> None:
        """
        ...
        for pattern in self.SQL_INJECTION_PATTERNS:
            if pattern.search(value):
                result.add_error(
                    f"Input contains potential SQL injection pattern: " f"{pattern.pattern}"
                )
